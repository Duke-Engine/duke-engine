...
class CommandCodecTest {
    ...
    private static void assertRoundTrips(CommandPacket packet) {
        var decoded = CommandCodec.decode(CommandCodec.encode(packet));
    ...
    void floatBitsArePreserved() {
        ...
        var decoded = (GameMessage.MoveTo) CommandCodec.decode(CommandCodec.encode(packet)).commands().get(0);
