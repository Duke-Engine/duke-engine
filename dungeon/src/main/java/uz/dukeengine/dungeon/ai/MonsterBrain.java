...
public final class MonsterBrain extends UnitScript {
    ...
    /** Its block: the word a unit's {@code Modules} list opens with, and nothing to say inside it. */
    ...
    /**
     * Which creature it is: the unit it was put on, looked up by name. A MonsterBrain on a Brute is the
     * Brute's brain because the block it sits in says Brute — there is no second name in the file to keep
     * in step with it.
     *
     * <p>By name rather than out of the template itself, because the tuning a brain reads is the
     * settings', and a caller may hand in re-tuned settings over the same creature file: see
     * {@code Dungeon.world(map, settings, creaturesIni)}.
     */
    @Override
    public void onStart() {
        kind = settings.monster(unit().getTemplate().name());
        if (kind == null) {
            throw new IllegalStateException("no monster is called " + unit().getTemplate().name()
                    + ", so a MonsterBrain on it has nothing to be the brain of");
        }
    }
